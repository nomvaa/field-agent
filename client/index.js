...
const handleEdit = (agentId) => {
    hideMessages();
    const index = agents.findIndex(a => a.agentId);
    if(index !== -1) {
...
const handleSaveAgent = (evt) => {
    ...
    if(currentAgent.agentId === 0) {
        ...
        .then(data => {
            displayMessage(`${data.title} was added!`, 'SUCCESS');
            refreshList();
            setView('list');
      })
      .catch(errors => {
...
const refreshList = () => {
  ...
  const renderAgents = async() => {
    ...
    for (let agent of agents) {
        ...
        <img class="card-img-top" src="https://upload.wikimedia.org/wikipedia/commons/a/ac/Default_pfp.jpg" alt="Card image cap">
            <div class="card-body">
